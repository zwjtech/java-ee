package com.changwen.springdata.dao;
 ...
 *
 * 3. 实际上, 也可以通过 @RepositoryDefinition 注解来替代继承 Repository 接口，一个属性pojo类型，另一属性是主键类型
 */
/**
...
//    List<Person> getByAddress_IdGreaterThan(Integer id);
    ...
    //为 @Query 注解传递参数的方式1: 使用占位符.(方法里的参数要按JQHL语句里顺序）
    @Query("SELECT p FROM Person p WHERE p.lastName = ?1 AND p.email = ?2")
    List<Person> testQueryAnnotationParams1(String lastName, String email);

    //为 @Query 注解传递参数的方式2: 命名参数的方式.（可以不用顺序，跟名字匹配就行）
